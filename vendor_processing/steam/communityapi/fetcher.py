...
def get_inventory_dummy():
    with open(
        "../api_responses/inventory_response.json", "r", encoding="utf-8"
    ) as file:
...
def fetch_some_items(db_session: Session, start: int, count: int):
    ...
    offer_data = list(
        map(
            lambda item: {
                ...
                "affiliate_link": "https://steamcommunity.com/market/listings/730/"
                + item["asset_description"]["market_hash_name"],
                "vendorid": 1,
            },
            items["results"],
    ...
    print(
        ...
        + " items from steam community market\n"
    )
...
def fetch_inventory(db_session: Session, steamid):
    ...
    for classid in descriptions.keys():
        ...
        if not vendor_offers_service.exists(
    ...
    newest_update_timestamp = assets_service.get_timestamp_of_last_update(
        db_session=db_session, steamid=steamid
    )

    assets_service.create_or_update(db_session=db_session, assets_in=assets)
...
def fetch_item_price(db_session: Session, market_hash_name, classid):
    ...
    vendor_offers_service.create_or_update_without_sell_listings(
        db_session=db_session, offers_in=offer
    )
...
def fetch_price_history(db_session: Session, market_hash_name, classid):
    ...
    history_listings = list(
        map(
            lambda x: {
                "classid": classid,
                "vendorid": 1,
                "time_stamp": datetime.strptime(
                    x[0].replace(": +0", ""), "%b %d %Y %H"
    ...
    price_histories_service.create_or_update(
        db_session=db_session, history_listing_in=history_listings
    )
    print("\n")
